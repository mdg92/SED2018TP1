...
Model &Farmer::externalFunction(const ExternalMessage &msg)
{
	...
	if(this->received[0] && this->received[1] && this->received[2] && this->received[3] && this->received[4]){
		...
		if(this->past_r_usd[0]<this->past_cost[0])																								this->s_usd =0;
		if(this->past_cost[0]<=this->past_r_usd[0] && this->past_r_usd[0] < ( ((this->wgc_status*0.2)+0.6)*0.1*954 ) )							this->s_usd =0.1;
		if( (( (this->wgc_status*0.2)+0.6)*0.1*954 )<=this->past_r_usd[0] && this->past_r_usd[0] < ( ((this->wgc_status*0.2)+0.6)*0.2*954 ) )  	this->s_usd =0.2;
		if( (( (this->wgc_status*0.2)+0.6)*0.2*954 )<=this->past_r_usd[0] && this->past_r_usd[0] < ( ((this->wgc_status*0.2)+0.6)*0.3*954 ) )  	this->s_usd =0.1;
		...
		if ( (this->s_usd*90.0+this->s_amb*10.0) < dice && (this->past_r_usd[0]/this->past_cost[0])<(this->past_r_usd[1]/this->past_cost[1])) this->field_saved = this->field_neighbour;
		holdIn(AtomicState::active, VTime(0,0,0,0));
	}else{
		passivate();
	}
...
Model &Farmer::outputFunction(const CollectMessage &msg)
{
	...
	this->sendOutput(msg.time(), satisfaction, (this->s_usd*90.0+this->s_amb*10.0));
	return *this;
}